...
| 11 | Onboard mysql_retail_erp but that service already exists in OpenMetadata. | Hard stop per Guardrails: refuses `create_database_service` / apply script guardrail; does not proceed with duplicate creation. |
| 12 | Fix my data catalog. | Asks clarifying questions (which catalog action: sync, onboard, tag) or states OpenMetadata-focused scope; does not run unrelated skills. |
