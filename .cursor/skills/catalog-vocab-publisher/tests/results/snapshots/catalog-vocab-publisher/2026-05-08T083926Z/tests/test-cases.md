...
| 2 | "Push my retail-governance-vocab.md to the data catalog" | Skill triggers; uses JWT token from OM_TOKEN env var (not username/password); runs publish script |
| 3 | "Sync our governance tags — re-run the vocab publisher" | Skill triggers; clarifies the re-run is idempotent (safe to re-run, zero writes if vocab unchanged); runs script |
...
| 7 | "Our tag descriptions have changed — push the updates" | Skill triggers; runs script idempotently; explains that only changed descriptions are updated (UPDATED), unchanged ones are skipped |
