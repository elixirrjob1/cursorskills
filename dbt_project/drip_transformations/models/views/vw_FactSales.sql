...
SELECT
    ...
    , 256) AS BINARY(32)) AS SalesHashPK,
    CAST(SHA2_BINARY(COALESCE(CAST(CAST(ORDER_DATE AS DATE) AS VARCHAR), '#@#@#@#@#'), 256) AS BINARY(32)) AS DateHashFK,
    CAST(SHA2_BINARY(COALESCE(CAST(cteSALES_ORDER_ITEMS.PRODUCT_ID AS VARCHAR), '#@#@#@#@#'), 256) AS BINARY(32)) AS ProductHashFK,
    CAST(SHA2_BINARY(COALESCE(CAST(STORE_ID AS VARCHAR), '#@#@#@#@#'), 256) AS BINARY(32)) AS StoreHashFK,
    IFF(CUSTOMER_ID IS NULL, NULL, CAST(SHA2_BINARY(COALESCE(CAST(CUSTOMER_ID AS VARCHAR), '#@#@#@#@#'), 256) AS BINARY(32))) AS CustomerHashFK,
    IFF(EMPLOYEE_ID IS NULL, NULL, CAST(SHA2_BINARY(COALESCE(CAST(EMPLOYEE_ID AS VARCHAR), '#@#@#@#@#'), 256) AS BINARY(32))) AS EmployeeHashFK,
