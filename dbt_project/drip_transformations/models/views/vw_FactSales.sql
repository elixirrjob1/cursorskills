...
SELECT
    ...
    HASH(COALESCE(CAST(cteSALES_ORDER_ITEMS.PRODUCT_ID AS VARCHAR), '#@#@#@#@#')
        || '|' || 'ERP') AS "ProductHashFK",
    HASH(COALESCE(CAST(STORE_ID AS VARCHAR), '#@#@#@#@#')) AS "StoreHashFK",
    IFF(CUSTOMER_ID IS NULL, NULL, HASH(COALESCE(CAST(CUSTOMER_ID AS VARCHAR), '#@#@#@#@#')
        || '|' || 'ERP')) AS "CustomerHashFK",
