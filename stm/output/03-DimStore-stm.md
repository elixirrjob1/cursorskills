...
|--------------|---------------|-----------|------------|---------------|--------------|------------------|--------------------------------|-----------|--------------------|-------------|
| DimStore | StoreHashPK | INT | Primary Key | Snowflake | STORES | STORE_ID | SHA2(COALESCE(CAST(STORE_ID AS VARCHAR), '#@#@#@#@#'), 256) | NO |  | Surrogate primary key for store dimension |
| DimStore | StoreHashBK | VARCHAR(10) | Business Key | Snowflake | STORES | CODE | SHA2(COALESCE(CAST(CODE AS VARCHAR), '#@#@#@#@#'), 256) | NO |  | Natural business key (store number) |
