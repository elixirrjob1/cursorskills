...
|--------------|---------------|-----------|------------|---------------|--------------|------------------|--------------------------------|-----------|--------------------|-------------|
| DimSupplier | SupplierHashPK | INT | Primary Key | Snowflake | SUPPLIERS | SUPPLIER_ID | SHA2_BINARY(COALESCE(CAST(SUPPLIER_ID AS VARCHAR), '#@#@#@#@#'), 256) | NO |  | Surrogate primary key for supplier dimension |
| DimSupplier | SupplierHashBK | VARCHAR(20) | Business Key | Snowflake | SUPPLIERS | SUPPLIER_ID | SHA2_BINARY(COALESCE(CAST(SUPPLIER_ID AS VARCHAR), '#@#@#@#@#'), 256) | NO |  | Natural business key (supplier ID from procurement system) |
