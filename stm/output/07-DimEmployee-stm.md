...
|--------------|---------------|-----------|------------|---------------|--------------|------------------|--------------------------------|-----------|--------------------|-------------|
| DimEmployee | EmployeeHashPK | NUMBER(19,0) | Primary Key | Snowflake | EMPLOYEES | EMPLOYEE_ID | HASH(COALESCE(CAST(EMPLOYEE_ID AS VARCHAR), '#@#@#@#@#')) | NO |  | Surrogate primary key for employee dimension |
| DimEmployee | EmployeeHashBK | NUMBER(19,0) | Business Key | Snowflake | EMPLOYEES | EMPLOYEE_ID | HASH(COALESCE(CAST(EMPLOYEE_ID AS VARCHAR), '#@#@#@#@#')) | NO |  | Natural business key (employee ID from HR system) |
...
| DimEmployee | ManagerEmployeeHashFK | NUMBER(19,0) | Foreign Key | Snowflake |  |  | IFF({SOURCE_COL} IS NULL, NULL, HASH(COALESCE(CAST({SOURCE_COL} AS VARCHAR), '#@#@#@#@#'))) | YES |  | Foreign key to manager employee record |
| DimEmployee | HomeStoreHashFK | NUMBER(19,0) | Foreign Key | Snowflake | EMPLOYEES | STORE_ID | IFF(STORE_ID IS NULL, NULL, HASH(COALESCE(CAST(STORE_ID AS VARCHAR), '#@#@#@#@#'))) | YES |  | Foreign key to employee primary store location |
