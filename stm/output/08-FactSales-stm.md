...
|--------------|---------------|-----------|------------|---------------|--------------|------------------|--------------------------------|-----------|--------------------|-------------|
| FactSales | SalesHashPK | INT | Primary Key | Snowflake | SALES_ORDER_ITEMS | SALES_ORDER_ID, SALES_ORDER_ITEM_ID | CAST(SHA2_BINARY(COALESCE(CAST(SALES_ORDER_ID AS VARCHAR), '#@#@#@#@#') \|\| '\|' \|\| COALESCE(CAST(SALES_ORDER_ITEM_ID AS VARCHAR), '#@#@#@#@#'), 256) AS BINARY(32)) | NO |  | Surrogate primary key for sales fact |
| FactSales | DateHashFK | INT | Foreign Key | Snowflake | SALES_ORDERS | ORDER_DATE | CAST(SHA2_BINARY(COALESCE(CAST(CAST(ORDER_DATE AS DATE) AS VARCHAR), '#@#@#@#@#'), 256) AS BINARY(32)) | NO |  | Foreign key to date dimension (transaction date) |
| FactSales | ProductHashFK | INT | Foreign Key | Snowflake | SALES_ORDER_ITEMS | PRODUCT_ID | CAST(SHA2_BINARY(COALESCE(CAST(PRODUCT_ID AS VARCHAR), '#@#@#@#@#'), 256) AS BINARY(32)) | NO |  | Foreign key to product dimension |
| FactSales | StoreHashFK | INT | Foreign Key | Snowflake | SALES_ORDERS | STORE_ID | CAST(SHA2_BINARY(COALESCE(CAST(STORE_ID AS VARCHAR), '#@#@#@#@#'), 256) AS BINARY(32)) | NO |  | Foreign key to store dimension |
| FactSales | CustomerHashFK | INT | Foreign Key | Snowflake | SALES_ORDERS | CUSTOMER_ID | IFF(CUSTOMER_ID IS NULL, NULL, CAST(SHA2_BINARY(COALESCE(CAST(CUSTOMER_ID AS VARCHAR), '#@#@#@#@#'), 256) AS BINARY(32))) | YES |  | Foreign key to customer dimension (NULL for anonymous sales) |
| FactSales | EmployeeHashFK | INT | Foreign Key | Snowflake | SALES_ORDERS | EMPLOYEE_ID | IFF(EMPLOYEE_ID IS NULL, NULL, CAST(SHA2_BINARY(COALESCE(CAST(EMPLOYEE_ID AS VARCHAR), '#@#@#@#@#'), 256) AS BINARY(32))) | YES |  | Foreign key to employee dimension (sales associate) |
